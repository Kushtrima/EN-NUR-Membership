...
<body>
    <div class="container">
        ...
        <table style="width: 100%; margin: 40px 0; border-collapse: collapse;">
            <tr>
                <td style="width: 33%; vertical-align: top; padding-right: 20px; font-size: 12px; color: #000;">
                    ...
                    <div style="margin-bottom: 4px;">Data e pagesës: {{ $payment->created_at->format('F j, Y') }}</div>
        ...
        <div style="border-top: 2px solid #1F6E38; padding-top: 15px; margin: 30px 0;">
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="vertical-align: top; font-size: 13px; color: #000;">
                        <div style="margin-bottom: 5px;">PERIUDHA E VLEFSHME E ANËTARËSISË</div>
                        <div style="font-size: 16px; margin-bottom: 3px;">{{ $payment->created_at->format('M j, Y') }} – {{ $payment->created_at->addYear()->format('M j, Y') }}</div>
                        <div style="font-size: 12px; color: #666;">365 Ditë të Plota Qasje</div>
                    </td>
                    <td style="text-align: right; vertical-align: top; font-size: 13px; color: #000;">
                        <div style="font-size: 24px; margin-bottom: 3px; color: #000;">CHF {{ (int)($payment->amount / 100) }}</div>
                        <div style="font-size: 12px; color: #666;">Tarifë Vjetore Anëtarësie</div>
                    </td>
                </tr>
            </table>
        </div>
                ...
                <tbody>
                    <tr>
                        <td>
                            ...
                            <small style="color: #666;">{{ $payment->created_at->format('M j, Y') }} – {{ $payment->created_at->addYear()->format('M j, Y') }}</small>
                        </td>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="text-align: left; font-size: 11px; color: #666; vertical-align: top;">
                        <p style="margin: 0;">Faleminderit që u bashkuat me komunitetin tonë! Kjo faturë konfirmon anëtarësinë tuaj vjetore.</p>

...
require __DIR__.'/auth.php'; 
    ...
    Route::get('/debug-email-config', function () {
        try {
            $output = [];
            $output[] = "🔧 EMAIL CONFIGURATION DEBUG";
            $output[] = "Timestamp: " . now()->toDateTimeString();
            $output[] = "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━";
            $output[] = "";
            ...
            $output[] = "- MAIL_PORT: " . config('mail.mailers.smtp.port');
            $output[] = "- MAIL_USERNAME: " . config('mail.mailers.smtp.username');
            $output[] = "- MAIL_PASSWORD: " . (config('mail.mailers.smtp.password') ? '[SET - Length: ' . strlen(config('mail.mailers.smtp.password')) . ']' : '[NOT SET]');
            $output[] = "- MAIL_ENCRYPTION: " . config('mail.mailers.smtp.encryption');
            $output[] = "- MAIL_FROM_ADDRESS: " . config('mail.from.address');
            $output[] = "- MAIL_FROM_NAME: " . config('mail.from.name');
            $output[] = "";
            
            // Test email sending using exact same logic as working admin notifications
            $output[] = "📤 TESTING EMAIL SEND (Same logic as admin notifications):";
            
            // Find a test user (infinitdizzajn@gmail.com)
            $testUser = \App\Models\User::where('email', 'infinitdizzajn@gmail.com')->first();
            if (!$testUser) {
                $output[] = "❌ Test user infinitdizzajn@gmail.com not found";
            } else {
                $output[] = "✅ Test user found: {$testUser->name} ({$testUser->email})";
                
                // Create test message using same format as bulk notifications
                $subject = "TEST: Email Configuration Debug - " . now()->format('H:i:s');
                $message = "Dear {$testUser->name},\n\n";
                $message .= "This is a test email to debug the email configuration.\n\n";
                $message .= "EMAIL DEBUG DETAILS:\n";
                $message .= "━━━━━━━━━━━━━━━━━━━━\n";
                $message .= "• Test Time: " . now()->toDateTimeString() . "\n";
                $message .= "• Mail Driver: " . config('mail.default') . "\n";
                $message .= "• SMTP Host: " . config('mail.mailers.smtp.host') . "\n";
                $message .= "• SMTP Port: " . config('mail.mailers.smtp.port') . "\n";
                $message .= "• From Address: " . config('mail.from.address') . "\n\n";
                ...
                try {
                    // Use EXACT same email sending logic as AdminController::sendBulkNotifications
                    Mail::raw($message, function ($mail) use ($testUser, $subject) {
                        $mail->to($testUser->email, $testUser->name)
                             ->subject($subject)
                             ->from(config('mail.from.address'), config('mail.from.name'))
                    ...
                    $output[] = "✅ EMAIL SENT SUCCESSFULLY!";
                    $output[] = "- To: {$testUser->email}";
                    $output[] = "- Subject: {$subject}";
                    $output[] = "- From: " . config('mail.from.address');
                    $output[] = "";
                    $output[] = "🎯 Check the inbox for infinitdizzajn@gmail.com";
                    $output[] = "If this email arrives, the bulk notifications should work too!";
                    ...
                    if (strpos($e->getMessage(), '550 5.7.1') !== false) {
                        $output[] = "🔍 RELAYING DENIED - Possible solutions:";
                        $output[] = "1. Check if MAIL_PASSWORD_SECRET is correct in Render";
                        $output[] = "2. Verify Zoho app password is still valid";
                        $output[] = "3. Ensure using correct SMTP server (smtp.zoho.eu)";
                        $output[] = "4. Check if port 587 with TLS is configured";
                    } elseif (strpos($e->getMessage(), 'Connection refused') !== false) {
                        $output[] = "🔍 CONNECTION REFUSED - Check:";
                        $output[] = "1. SMTP host and port configuration";
                        $output[] = "2. Network connectivity to SMTP server";
                    } elseif (strpos($e->getMessage(), 'timeout') !== false) {
                        $output[] = "🔍 TIMEOUT - Check:";
                        $output[] = "1. SMTP server responsiveness";
                        $output[] = "2. Port accessibility";
                    }
                }
            }
            
            $output[] = "";
            $output[] = "🎯 RECOMMENDED WORKING CONFIGURATION:";
            $output[] = "Based on previous successful tests:";
            $output[] = "- MAIL_HOST=smtp.zoho.eu";
            $output[] = "- MAIL_PORT=587";
            $output[] = "- MAIL_ENCRYPTION=tls";
            $output[] = "- MAIL_PASSWORD=h261htHyeHBH (latest working app password)";
            
            return response('<h2>Email Configuration Debug Results</h2><pre>' . implode("\n", $output) . '</pre><br><br><a href="/dashboard" style="background: #28a745; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Go to Dashboard</a>');
            
        } catch (\Exception $e) {
            return response('<h2>Email Debug Failed</h2><pre>Error: ' . $e->getMessage() . "\n\nTrace:\n" . $e->getTraceAsString() . '</pre><br><a href="/dashboard">Go to Dashboard</a>');
        }
    });

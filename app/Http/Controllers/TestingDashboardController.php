...
class TestingDashboardController extends Controller
{
    ...
    /**
     * Get system logs from the last 30 days with automatic cleanup
     */
    public function getSystemLogs()
    {
        try {
            ...
            return response()->json([
                'success' => false,
                'error' => 'Failed to retrieve system logs: ' . $e->getMessage()
            ]);
        }
    }
    ...
    private function cleanupOldLogs()
    {
        try {
            ...
            if (is_dir($logsDirectory)) {
                $files = glob($logsDirectory . '/*.log');
                
                foreach ($files as $file) {
                    $fileTime = Carbon::createFromTimestamp(filemtime($file));
                    
                    // Delete files older than 30 days (except current laravel.log)
                    if ($fileTime->lt($cutoffDate) && basename($file) !== 'laravel.log') {
                        unlink($file);
                        Log::info("Cleaned up old log file: " . basename($file));
                    }
                }
                
                // Clean up the main laravel.log file if it's too large
                $mainLogPath = storage_path('logs/laravel.log');
                if (file_exists($mainLogPath) && filesize($mainLogPath) > 50 * 1024 * 1024) { // 50MB
                    $this->truncateLogFile($mainLogPath, $cutoffDate);
                }
            }
    ...
    /**
     * Truncate log file to keep only entries from the last 30 days
     */
    private function truncateLogFile($logPath, $cutoffDate)
    {
        try {
            $logContent = file_get_contents($logPath);
            $logLines = explode("\n", $logContent);
            ...
            foreach ($logLines as $line) {
                ...
                if (preg_match('/^\[(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\]/', $line, $matches)) {
                    ...
                    // Keep lines that don't match the timestamp pattern (might be stack traces)
                    $filteredLines[] = $line;
                }
            }
            ...
            Log::info("Truncated main log file to last 30 days");
            
        } catch (\Exception $e) {
            Log::error('Failed to truncate log file: ' . $e->getMessage());
        }
    }
     ...
     */
    private function extractLogContext($message)
    {
        // Extract useful context from common log patterns
        if (strpos($message, 'Exception') !== false || strpos($message, 'Error') !== false) {
        ...
        if (strpos($message, 'Failed to send') !== false) {
